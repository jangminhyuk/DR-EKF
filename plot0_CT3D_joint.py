#!/usr/bin/env python3
"""
plot0_CT3D_joint.py: Trajectory visualization for EKF vs DR-EKF comparison results from main0_CT3D_joint.py
...
def load_data(results_path, dist):
    """Load saved results from main0_CT3D_joint.py experiments."""
...
def load_detailed_results_for_filter(results_path, filter_name, theta_vals, dist):
    ...
    if filter_name == 'EKF':
        ...
        filename = f'detailed_results_{filter_name}_teps0{theta_vals["theta_eps0"]:.4f}_teps{theta_vals["theta_eps"]:.4f}_{dist}.pkl'
    else:
...
def generate_desired_trajectory(T_total=10.0, dt=0.2):
    """Generate desired trajectory matching main0_CT3D_joint.py (3D CT model)."""
...
def extract_trajectory_data_from_saved(optimal_results, results_path, dist):
    ...
    filters_order = ['EKF', 'DR_EKF_TAC', 'DR_EKF_CDC', 'DR_EKF_CDC_FW', 'DR_EKF_joint']
    trajectory_data = {}
    ...
    for filt in filters_order:
        ...
        if filt == 'EKF':
            ...
            theta_str = f"θ_ε₀={theta_vals['theta_eps0']}, θ_ε={theta_vals['theta_eps']}"
...
def plot_trajectory_subplots(trajectory_data, filters_order, desired_traj, time, dist):
    ...
    results_dir = os.path.join("results", "EKF_comparison_joint_CT3D")
    ...
    for filename in saved_files:
        print(f"- ./results/EKF_comparison_joint_CT3D/{filename}")
...
def plot_subplots_all_filters(trajectory_data, filters_order, desired_traj, time, dist):
    ...
    # --- XY Trajectories Figure (3x2 grid for 5 filters) ---
    fig_xy, axes_xy = plt.subplots(3, 2, figsize=(12, 15))
    axes_xy = axes_xy.flatten()

    # --- Altitude vs Time Figure (3x2 grid for 5 filters) ---
    fig_pz, axes_pz = plt.subplots(3, 2, figsize=(12, 15))
    ...
    print(f"- ./results/EKF_comparison_joint_CT3D/traj_3d_EKF_subplots_xy_{dist}.pdf")
    print(f"- ./results/EKF_comparison_joint_CT3D/traj_3d_EKF_subplots_pz_{dist}.pdf")
...
def plot_mse_heatmaps(all_results, dist):
    ...
    # Plot Joint DR-EKF filter (theta_eps0 vs theta_eps)
    if 'DR_EKF_joint' in all_results and all_results['DR_EKF_joint']:
        filt = 'DR_EKF_joint'
        ...
        for theta_key, results in all_results[filt].items():
            ...
            if teps0 not in theta_data:
                theta_data[teps0] = {}
            theta_data[teps0][teps] = results['mse_mean']

        # Create grid
        teps0_vals = sorted(theta_data.keys())
        teps_vals = sorted(set(te for te0_dict in theta_data.values() for te in te0_dict.keys()))

        # Create MSE matrix
        mse_matrix = np.zeros((len(teps_vals), len(teps0_vals)))
        for i, teps in enumerate(teps_vals):
            for j, teps0 in enumerate(teps0_vals):
                if teps in theta_data[teps0]:
                    mse_matrix[i, j] = theta_data[teps0][teps]
                else:
                    mse_matrix[i, j] = np.nan
        ...
        ax.set_xticklabels([f'{te0:.4f}' for te0 in teps0_vals], fontsize=14)
        ax.set_yticklabels([f'{te:.4f}' for te in teps_vals], fontsize=14)
...
def main():
    parser = argparse.ArgumentParser(description='Create 3D trajectory plots for EKF vs DR-EKF comparison from main0_CT3D_joint.py (3D CT model with joint DR-EKF)')
    ...
    try:
        ...
        if not args.heatmaps_only:
            ...
            if len(trajectory_data) == 0:
                print("No trajectory data found. You need to run main0_CT3D_joint.py first to generate the detailed trajectory files.")
            else:
        ...
        print(f"Make sure you have run main0_CT3D_joint.py with --dist {args.dist} first")
