#!/usr/bin/env python3
"""
plot0_uni_joint.py: Trajectory visualization for EKF vs DR-EKF comparison results from main0_uni_joint.py
Creates 2D trajectory plots with 1-std uncertainty tubes.
Includes joint DR-EKF with θ_ε display.
"""
...
def load_data(results_path, dist):
    """Load saved results from main0_uni_joint.py experiments."""
...
def load_detailed_results(results_path, theta_val, dist):
    """Load detailed trajectory data for non-joint filters at a specific theta value."""
...
def load_detailed_results_for_joint(results_path, theta_eps0, theta_eps, dist):
    """Load detailed trajectory data for joint DR-EKF filter."""
    detailed_file = os.path.join(results_path,
        f'detailed_results_DR_EKF_joint_teps0{theta_eps0}_teps{theta_eps}_{dist}.pkl')
...
def generate_desired_trajectory(T_total=10.0, dt=0.2):
    """Generate desired trajectory matching main0_uni_joint.py."""
...
def extract_trajectory_data_from_saved(optimal_results, results_path, dist):
    ...
    filters_order = ['EKF', 'DR_EKF_CDC', 'DR_EKF_TAC', 'DR_EKF_CDC_FW', 'DR_EKF_joint']
    trajectory_data = {}
    ...
    for filt in filters_order:
        ...
        if filt == 'DR_EKF_joint':
            theta_eps0 = optimal_stats['theta_eps0']
            theta_eps = optimal_stats['theta_eps']
            theta_str = f"θ_ε₀={theta_eps0}, θ_ε={theta_eps}"
            print(f"Loading trajectory data for {filt} with {theta_str}")
        else:
            optimal_theta = optimal_stats['theta']
            theta_str = f"θ*={optimal_theta}"
            print(f"Loading trajectory data for {filt} with {theta_str}")

        try:
            # Load detailed results based on filter type
            if filt == 'DR_EKF_joint':
                detailed_results = load_detailed_results_for_joint(
                    results_path, theta_eps0, theta_eps, dist)
            else:
                detailed_results = load_detailed_results(
                    results_path, optimal_stats['theta'], dist)
...
def plot_trajectory_subplots(trajectory_data, filters_order, desired_traj, time, dist):
    ...
    filter_names = {
        ...
        'DR_EKF_joint': "DR-EKF (Joint)"
    }

    # Create results directory
    results_dir = os.path.join("results", "EKF_comparison_joint_uni")
    ...
    for filename in saved_files:
        print(f"- ./results/EKF_comparison_joint_uni/{filename}")
...
def plot_subplots_all_filters(trajectory_data, filters_order, desired_traj, time, dist):
    ...
    # Create subplot layout - 3x2 grid for up to 6 filters (5 used)
    ...
    for idx in range(len(available_filters), 6):
        if idx < len(axes):
    ...
    print(f"EKF subplots trajectory figure saved to: ./results/EKF_comparison_joint_uni/traj_2d_EKF_subplots_{dist}.pdf")

def plot_performance_vs_robustness(all_results, dist):
    """Plot MSE and tracking cost vs robustness parameters for non-joint filters."""
    if all_results is None:
        print("Warning: No all_results data available. Cannot plot performance vs robustness parameters.")
        return

    # Extract theta values (exclude 'joint_sweep' key)
    theta_values = sorted([k for k in all_results.keys() if k != 'joint_sweep'])
    non_joint_filters = ['EKF', 'DR_EKF_CDC', 'DR_EKF_TAC', 'DR_EKF_CDC_FW']
    ...
    print(f"Performance vs robustness plot saved to: ./results/EKF_comparison_joint_uni/performance_vs_robustness_{dist}.pdf")

    # Create separate individual plots for publication
    create_individual_performance_plots(performance_data, non_joint_filters, colors, filter_names, dist)
...
def create_individual_performance_plots(performance_data, filters_order, colors, filter_names, dist):
    ...
    print(f"Individual MSE plot saved to: ./results/EKF_comparison_joint_uni/mse_vs_robustness_{dist}.pdf")
    print(f"Individual cost plot saved to: ./results/EKF_comparison_joint_uni/cost_vs_robustness_{dist}.pdf")

def plot_joint_heatmaps(all_results, dist):
    """Create heatmap plots for joint DR-EKF: MSE and cost vs theta_eps0 x theta_eps."""
    if all_results is None or 'joint_sweep' not in all_results:
        print("Warning: No joint sweep data available. Cannot plot joint heatmaps.")
        return
    ...
    joint_data = all_results['joint_sweep']
    if not joint_data:
        print("Warning: Joint sweep data is empty.")
        return

    # Extract theta_eps0 and theta_eps values
    theta_data_mse = {}
    theta_data_cost = {}
    for theta_key, results in joint_data.items():
        teps0, teps = theta_key
        if teps0 not in theta_data_mse:
            theta_data_mse[teps0] = {}
            theta_data_cost[teps0] = {}
        theta_data_mse[teps0][teps] = results['mse_mean']
        theta_data_cost[teps0][teps] = results['cost_mean']

    # Create grid
    teps0_vals = sorted(theta_data_mse.keys())
    teps_vals = sorted(set(te for te0_dict in theta_data_mse.values() for te in te0_dict.keys()))

    for metric, theta_data, metric_label in [
        ('mse', theta_data_mse, 'MSE'),
        ('cost', theta_data_cost, 'Tracking Cost')
    ]:
        # Create matrix
        matrix = np.zeros((len(teps_vals), len(teps0_vals)))
        for i, teps in enumerate(teps_vals):
            for j, teps0 in enumerate(teps0_vals):
                if teps in theta_data[teps0]:
                    matrix[i, j] = theta_data[teps0][teps]
                else:
                    matrix[i, j] = np.nan
        ...
        use_log = (metric == 'mse')
        if use_log:
            im = ax.imshow(matrix, aspect='auto', cmap='viridis', origin='lower', norm=LogNorm())
        else:
            im = ax.imshow(matrix, aspect='auto', cmap='viridis', origin='lower')

        # Set ticks
        ax.set_xticks(np.arange(len(teps0_vals)))
        ax.set_yticks(np.arange(len(teps_vals)))
        ax.set_xticklabels([f'{te0}' for te0 in teps0_vals], fontsize=14)
        ax.set_yticklabels([f'{te}' for te in teps_vals], fontsize=14)

        ax.set_xlabel('θ_ε₀', fontsize=18)
        ax.set_ylabel('θ_ε', fontsize=18)
        ax.set_title(f'{metric_label} Heatmap: DR-EKF (Joint)', fontsize=20)
        ...
        for i in range(len(teps_vals)):
            for j in range(len(teps0_vals)):
                if not np.isnan(matrix[i, j]):
                    text = ax.text(j, i, f'{matrix[i, j]:.3f}',
                                 ha="center", va="center", color="w", fontsize=14)

        plt.tight_layout()
        save_path = os.path.join(results_dir, f"{metric}_heatmap_DR_EKF_joint_{dist}.pdf")
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        plt.close(fig)
        print(f"Saved {metric_label} heatmap: {save_path}")
...
def create_violin_plots(optimal_results, results_path, dist):
    ...
    for metric in ['mse', 'cost']:
        ...
        for filt in filters_order:
            ...
            # Determine theta string for label
            if filt == 'EKF':
                theta_str = None
            elif filt == 'DR_EKF_joint':
                theta_eps0 = optimal_stats['theta_eps0']
                theta_eps = optimal_stats['theta_eps']
                theta_str = f"θ_ε₀={theta_eps0}, θ_ε={theta_eps}"
            else:
                theta_str = f"θ*={optimal_stats['theta']}"
            ...
            try:
                ...
                if filt == 'DR_EKF_joint':
                    ...
                    optimal_theta = optimal_stats['theta']
                    detailed_results = load_detailed_results(
                        results_path, optimal_theta, dist)

                if filt in detailed_results:
                    results = detailed_results[filt]['results']
                    ...
                    if len(raw_data) > 0:
                        ...
                        pos_counter += 1
                        print(f"Loaded {len(raw_data)} {metric} samples for {filt}")
...
def print_optimal_results_summary(optimal_results):
    ...
    for filter_name, results in optimal_results.items():
        ...
        if filter_name == 'DR_EKF_joint':
            theta_eps0 = results.get('theta_eps0', 'N/A')
            theta_eps = results.get('theta_eps', 'N/A')
            theta_str = f"θ_ε₀={theta_eps0}, θ_ε={theta_eps}"
        ...
        print(f"{filter_name:20s}: {theta_str:30s} MSE={mse_mean:.6f}±{mse_std:.6f}, Cost={cost_mean:.3f}±{cost_std:.3f}")

def main():
    parser = argparse.ArgumentParser(description='Create trajectory plots for EKF vs DR-EKF comparison from main0_uni_joint.py (unicycle with joint DR-EKF)')
    ...
    parser.add_argument('--heatmaps_only', action='store_true',
                        help='Create only joint heatmaps, skip trajectory plots')
    parser.add_argument('--all_plots', action='store_true',
                        help='Create all types of plots (trajectory + performance + violin + heatmap)')
    args = parser.parse_args()

    try:
        # Load optimal results and all results
        results_path = "./results/EKF_comparison_joint_uni/"
        ...
        # Always create performance vs robustness plots (non-joint filters)
        print(f"\nCreating performance vs robustness parameter plots...")
        plot_performance_vs_robustness(all_results, args.dist)

        # Always create joint heatmaps
        print(f"\nCreating joint DR-EKF heatmaps...")
        plot_joint_heatmaps(all_results, args.dist)
        ...
        if not args.heatmaps_only:
            ...
            if len(trajectory_data) == 0:
                print("No trajectory data found. You need to run main0_uni_joint.py first to generate the detailed trajectory files.")
            else:
        ...
        print(f"Make sure you have run main0_uni_joint.py with --dist {args.dist} first")
