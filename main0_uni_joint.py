...
def run_experiment(exp_idx, dist, num_sim, seed_base, robust_val, filters_to_execute, reference_traj,
                  nominal_params, true_params, num_samples=100, theta_eps0=None, theta_eps=None):
...
def main(dist, num_sim, num_exp, T_total=10.0, num_samples=100):
    ...
    theta_eps0_vals = [0.05]
    theta_eps_vals = [0.01, 0.05, 0.1, 0.5, 1.0, 5.0, 10.0]
    filters_to_execute = ['EKF', 'DR_EKF_CDC', 'DR_EKF_TAC', 'DR_EKF_CDC_FW', 'DR_EKF_joint']
    ...
    # Filters that use a single robust_val for all theta parameters
    non_joint_filters = [f for f in filters_to_execute if f != 'DR_EKF_joint']
    ...
    for robust_val in robust_vals:
        ...
        experiments = Parallel(n_jobs=-1, backend='loky')(
            delayed(run_experiment)(exp_idx, dist, num_sim, seed_base, robust_val,
                                   non_joint_filters, reference_traj, nominal_params, true_params, num_samples)
            for exp_idx in range(num_exp)
        )

        # Aggregate results
        aggregated = {filter_name: {'mse': [], 'cost': []} for filter_name in non_joint_filters}

        for exp in experiments:
            for filter_name in non_joint_filters:
        ...
        if robust_val in [0.01, 0.05, 0.1, 0.5, 1.0, 5.0, 10.0]:  # Save for all theta values
            ...
            detailed_results_dir = "./results/EKF_comparison_joint_uni/"
    ...
    # Run joint DR-EKF filter sweeping theta_eps0 × theta_eps directly
    from itertools import product
    joint_theta_combinations = list(product(theta_eps0_vals, theta_eps_vals))
    joint_results = {}  # (theta_eps0, theta_eps) -> results

    print(f"\nRunning Joint DR-EKF sweep: {len(joint_theta_combinations)} combinations (theta_eps0 × theta_eps)")
    for theta_eps0_val, theta_eps_val in joint_theta_combinations:
        print(f"\nRunning Joint DR-EKF with θ_ε₀={theta_eps0_val}, θ_ε={theta_eps_val}")

        experiments = Parallel(n_jobs=-1, backend='loky')(
            delayed(run_experiment)(exp_idx, dist, num_sim, seed_base, 0.0,
                                   ['DR_EKF_joint'], reference_traj, nominal_params, true_params, num_samples,
                                   theta_eps0=theta_eps0_val, theta_eps=theta_eps_val)
            for exp_idx in range(num_exp)
        )
        ...
        for exp in experiments:
            if 'DR_EKF_joint' in exp:
                aggregated_mse.append(exp['DR_EKF_joint']['mse_mean'])
                aggregated_cost.append(exp['DR_EKF_joint']['cost_mean'])
                if 'results' in exp['DR_EKF_joint']:
                    aggregated_detailed_results.extend(exp['DR_EKF_joint']['results'])

        if aggregated_mse:
            theta_key = (theta_eps0_val, theta_eps_val)
            joint_results[theta_key] = {
                'mse_mean': np.mean(aggregated_mse),
                'mse_std': np.std(aggregated_mse),
                'cost_mean': np.mean(aggregated_cost),
                'cost_std': np.std(aggregated_cost),
                'theta_eps0': theta_eps0_val,
                'theta_eps': theta_eps_val,
                'results': aggregated_detailed_results
            }
            print(f"  DR_EKF_joint: MSE={np.mean(aggregated_mse):.4f}±{np.std(aggregated_mse):.4f}, "
                  f"Cost={np.mean(aggregated_cost):.4f}±{np.std(aggregated_cost):.4f}")

            # Save detailed results for this combination
            detailed_results_dir = "./results/EKF_comparison_joint_uni/"
            if not os.path.exists(detailed_results_dir):
                os.makedirs(detailed_results_dir)
            detailed_results_path = os.path.join(detailed_results_dir,
                f'detailed_results_DR_EKF_joint_teps0{theta_eps0_val}_teps{theta_eps_val}_{dist}.pkl')
            aggregated_detailed = {
                'DR_EKF_joint': {
                    'mse_mean': np.mean(aggregated_mse),
                    'cost_mean': np.mean(aggregated_cost),
    ...
    # Find optimal theta for each non-joint filter
    optimal_results = {}
    for filter_name in non_joint_filters:
        best_cost = np.inf
        best_theta = None
    ...
    # Find optimal theta_eps0/theta_eps for joint filter
    if joint_results:
        best_cost = np.inf
        best_key = None
        best_stats = None
        for theta_key, stats in joint_results.items():
            if stats['cost_mean'] < best_cost:
                best_cost = stats['cost_mean']
                best_key = theta_key
                best_stats = stats
        if best_key is not None:
            optimal_results['DR_EKF_joint'] = {
                'theta_eps0': best_stats['theta_eps0'],
                'theta_eps': best_stats['theta_eps'],
                **best_stats
            }
            print(f"DR_EKF_joint: Optimal θ_ε₀={best_stats['theta_eps0']}, θ_ε={best_stats['theta_eps']}, Cost={best_cost:.4f}")
    ...
    # Include joint_results in all_results for saving
    ...
    print(f"\nEKF vs DR-EKF comparison (with joint DR-EKF) completed. Results saved to {results_path}")

    print("\nFinal Results Summary:")
    print("{:<20} {:<25} {:<20} {:<20}".format("Filter", "Optimal θ", "MSE", "Tracking Cost"))
    print("-" * 85)
    for filter_name, stats in optimal_results.items():
        if filter_name == 'DR_EKF_joint':
            theta_str = f"ε₀={stats['theta_eps0']}, ε={stats['theta_eps']}"
        else:
            theta_str = str(stats['theta'])
        print("{:<20} {:<25} {:<20.4f} {:<20.4f}".format(
            filter_name, theta_str, stats['mse_mean'], stats['cost_mean']))
