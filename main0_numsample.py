#!/usr/bin/env python3
"""
Study the effect of num_samples parameter on EKF vs DR-EKF comparison using unicycle dynamics.
This script runs experiments with different num_samples values and saves optimal results for each filter.
"""
...
def run_single_num_samples_experiment(dist, num_sim, num_exp, T_total, num_samples):
    """Run complete experiment for a single num_samples value using direct sampling"""
    seed_base = 2024
    dt = 0.2
    ...
    # --- Nominal estimation via direct sampling ---
    print(f"Using direct sampling with {num_samples} samples for nominal parameter estimation...")
    
    # Use direct sampling with fixed seed for consistent results
    np.random.seed(seed_base + 999999)  # Fixed seed for reproducible sampling
    nominal_params = estimate_nominal_parameters(
        x0_mean, x0_cov, mu_w, Sigma_w, mu_v, Sigma_v, dist, num_samples,
        x0_max, x0_min, w_max, w_min, v_max, v_min, x0_scale, w_scale, v_scale)
    
    # # --- Alternative: Nominal estimation via EM from shared input-output dataset ---
    # # (Kept for future use - more sophisticated but computationally expensive)
    # print(f"Using shared dataset and subsampling first {num_samples} trajectories for nominal parameter estimation...")
    # 
    # # Subsample the first num_samples trajectories from shared dataset
    # u_data = u_data_shared[:num_samples]  # Shape: (num_samples, T, 2, 1)
    # y_data = y_data_shared[:num_samples]  # Shape: (num_samples, T+1, 2, 1)
    #
    # # EM estimation (Gaussian nominal approximation)
    ...
    # Find optimal theta for each filter for this num_samples
    optimal_results = {}
    for filter_name in filters_to_execute:
        best_cost = np.inf
        best_theta = None
    ...
    print(f"  Optimal results for num_samples = {num_samples}:")
    for filter_name, stats in optimal_results.items():
        print(f"    {filter_name}: θ={stats['theta']}, Cost={stats['cost_mean']:.4f}±{stats['cost_std']:.4f}, MSE={stats['mse_mean']:.4f}±{stats['mse_std']:.4f}")
    
    return optimal_results

def main(dist, num_sim, num_exp, T_total=10.0):
    """Main experiment routine for studying effect of num_samples"""
    print(f"Starting num_samples study for distribution: {dist}")
    print(f"Experiment settings: num_sim={num_sim}, num_exp={num_exp}, T_total={T_total}")
    
    num_sample_list = [5, 10, 15, 20, 25, 30]
    
    # Store optimal results for each num_samples value
    all_optimal_results = {}
    
    for num_samples in num_sample_list:
        print(f"\n{'='*50}")
        print(f"Running experiment with num_samples = {num_samples}")
        print(f"{'='*50}")
        
        # Run experiment with direct sampling approach
        optimal_results = run_single_num_samples_experiment(
            dist, num_sim, num_exp, T_total, num_samples)
        
        all_optimal_results[num_samples] = optimal_results
    
    # Save results in proper directory structure
    results_path = f"./results/numsample_study/"
    if not os.path.exists(results_path):
        os.makedirs(results_path)
    
    # Save all optimal results organized by num_samples
    save_data(os.path.join(results_path, f'optimal_results_vs_numsamples_{dist}.pkl'), all_optimal_results)
    
    # Create summary results organized by filter for easy plotting
    summary_results = {}
    filters = ['EKF', 'DR_EKF_CDC', 'DR_EKF_TAC']
    
    for filter_name in filters:
        summary_results[filter_name] = {
            'num_samples': [],
            'optimal_theta': [],
            'mse_mean': [],
            'mse_std': [],
        ...
        for num_samples in num_sample_list:
            if filter_name in all_optimal_results[num_samples]:
                stats = all_optimal_results[num_samples][filter_name]
                summary_results[filter_name]['num_samples'].append(num_samples)
                summary_results[filter_name]['optimal_theta'].append(stats['theta'])
                summary_results[filter_name]['mse_mean'].append(stats['mse_mean'])
                summary_results[filter_name]['mse_std'].append(stats['mse_std'])
                summary_results[filter_name]['cost_mean'].append(stats['cost_mean'])
                summary_results[filter_name]['cost_std'].append(stats['cost_std'])
    
    # Save summary for easy plotting
    save_data(os.path.join(results_path, f'summary_results_vs_numsamples_{dist}.pkl'), summary_results)
    
    print(f"\n{'='*70}")
    print("FINAL SUMMARY: Effect of num_samples on Filter Performance")
    print(f"{'='*70}")
    print(f"{'num_samples':<12} {'Filter':<15} {'Optimal θ':<12} {'MSE':<20} {'Tracking Cost':<20}")
    print("-" * 80)
    ...
    for num_samples in num_sample_list:
        for filter_name in filters:
            if filter_name in all_optimal_results[num_samples]:
                stats = all_optimal_results[num_samples][filter_name]
                print(f"{num_samples:<12} {filter_name:<15} {stats['theta']:<12} "
                      f"{stats['mse_mean']:.4f}±{stats['mse_std']:.4f}    "
                      f"{stats['cost_mean']:.4f}±{stats['cost_std']:.4f}")
        print("-" * 80)
    
    print(f"\nResults saved to: {results_path}")
    print(f"  - optimal_results_vs_numsamples_{dist}.pkl: Complete optimal results for each num_samples")
    print(f"  - summary_results_vs_numsamples_{dist}.pkl: Summary organized by filter for plotting")
    
    return all_optimal_results, summary_results

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Study effect of num_samples on EKF vs DR-EKF performance")
