...
def plot_trajectory_subplots(trajectory_data, filters_order, desired_traj, time, dist):
    ...
    results_dir = os.path.join("results", "EKF_comparison_model_uncertainty")
    ...
    for filt in filters_order:
        ...
        save_path = os.path.join(results_dir, f"model_traj_2d_EKF_{filt}_{dist}.pdf")
        plt.savefig(save_path, dpi=300, bbox_inches='tight', pad_inches=0.2)
        saved_files.append(f"model_traj_2d_EKF_{filt}_{dist}.pdf")
        plt.close(fig)
    
    print(f"\nEKF trajectory plots saved to:")
    for filename in saved_files:
        print(f"- ./results/EKF_comparison_model_uncertainty/{filename}")
...
def plot_subplots_all_filters(trajectory_data, filters_order, desired_traj, time, dist):
    ...
    # Save subplot figure
    results_dir = os.path.join("results", "EKF_comparison_model_uncertainty")
    save_path = os.path.join(results_dir, f"model_traj_2d_EKF_subplots_{dist}.pdf")
    plt.savefig(save_path, dpi=300, bbox_inches='tight', pad_inches=0.2)
    plt.close(fig)
    
    print(f"EKF subplots trajectory figure saved to: ./results/EKF_comparison_model_uncertainty/model_traj_2d_EKF_subplots_{dist}.pdf")
...
def plot_performance_vs_robustness(all_results, dist):
    ...
    save_path = os.path.join(results_dir, f"model_performance_vs_robustness_{dist}.pdf")
    plt.savefig(save_path, dpi=300, bbox_inches='tight', pad_inches=0.2)
    plt.close(fig)
    
    print(f"Performance vs robustness plot saved to: ./results/EKF_comparison_model_uncertainty/model_performance_vs_robustness_{dist}.pdf")
...
def create_individual_performance_plots(performance_data, filters_order, colors, filter_names, dist):
    ...
    save_path = os.path.join(results_dir, f"model_mse_vs_robustness_{dist}.pdf")
    plt.savefig(save_path, dpi=300, bbox_inches='tight', pad_inches=0.2)
    plt.close(fig)
    ...
    save_path = os.path.join(results_dir, f"model_cost_vs_robustness_{dist}.pdf")
    plt.savefig(save_path, dpi=300, bbox_inches='tight', pad_inches=0.2)
    plt.close(fig)
    
    print(f"Individual MSE plot saved to: ./results/EKF_comparison_model_uncertainty/model_mse_vs_robustness_{dist}.pdf")
    print(f"Individual cost plot saved to: ./results/EKF_comparison_model_uncertainty/model_cost_vs_robustness_{dist}.pdf")
...
def create_violin_plots(detailed_results, theta_vals, dist, optimal_results):
    ...
    for metric in ['mse', 'cost']:
        ...
        output_path = os.path.join(results_dir, f'model_violin_plot_{metric}_{dist}.pdf')
